...
## The current level being played (0 is main menu)
...
## Call validation subroutine to validate save file on game start
func _ready() -> void:
	validation()
...
func validation() -> void:
	...
	if FileAccess.file_exists("user://save_game.txt"):
		...
		# Only continue to validate save file if length is correct
		if validate_length:
		...
		if not save_file_valid:
			...
			save_file.close()
			# Force game to return to main menu
			current_level = 0
	...
	# Make a new save file
		...
		# Force game to return to main menu
		current_level = 0
	# If current_level is zero, no level is being started: go to main menu
	if current_level == 0:
		# Prevent error with invalid save file when player leaves main menu
		await get_tree().process_frame
		MainMenu.menu_load()
	else:
		_game_load()
	
	# Reset save file validation variables
	save_file_valid = true
	validate_length = false
	validate_semicolons = false
	validate_levels = false
	validate_strength_items = false
	validate_strength_variable = false
	validate_collectibles = false
...
func start_level(l:int) -> void:
	...
	# Validate save + update the player's begin attack strength and item collection status
	validation()
...
func end_level(l:int) -> void:
	...
	if FileAccess.file_exists("user://save_game.txt"):
		...
		if l == 1 or l == 2:
			...
			current_level = 0
			validation()
	# Will replace the file and return to main menu
	else:
		validation()
